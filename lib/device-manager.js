...
class DeviceManager {
    constructor(adapter, api, errorHandler) {
        ...
        this.deviceTypes = {
            ...
            'Relay Switch 2PM': {
                type: 'blinds',
                commands: ['open', 'close', 'pause', 'setBlindPosition'],
                states: { 
                    open: 'boolean',
                    close: 'boolean',
                    pause: 'boolean',
    ...
    async createDeviceState(deviceId, stateName, stateType, writable = false) {
        ...
        const roleMap = {
            ...
            'CO2': 'value.co2',
            'open': 'button.close.blind',
            'close': 'button.close.blind',
            'pause': 'button.stop'
        };

        //const regex = new RegExp(`'button'|'level'`, 'g');
        //if(roleMap[stateName] && roleMap[stateName].match(regex)) { writable = true; }
        if(roleMap[stateName] && (roleMap[stateName].match('button') || roleMap[stateName].match('level'))) { writable = true; }
    ...
    async handlePhysicalDeviceCommand(deviceId, command, value) {
        ...
        try {
            const device = this.devices.get(deviceId);
            if (device && device.type === 'Relay Switch 2PM') {
                await this.handleBlindsCommand(deviceId, command, value);
                return; // Ends here, all commands send
            }else {   
            // Map command to API format using CommandMapper
                commandData = this.commandMapper.mapCommand(command, value);
            }
            if (!commandData) {
    ...
    /**
     * Handle blinds device commands
     * @param {string} deviceId - Device identifier
     * @param {string} command - Command name to execute
     * @param {any} value - Command parameter value
     */
    async handleBlindsCommand(deviceId, command, value) {
        let commandData = null;
        // in case Blinds are moving or new command send, delete old Timer
        if (this.relayTimers.has(deviceId)) {
            clearTimeout(this.relayTimers.get(deviceId));
            this.relayTimers.delete(deviceId);
        }

        switch (command) {
            case('pause'):
                // PAUSE: Send turnOff an both relais one after another
                await this.handleBlindsPause(deviceId, 'stop(1,2)');
                commandData = null;  // reset
                break; 
            case('setBlindPosition'):  // button
                // blind Position (value) SP is not button but 'slidePosition'
                const _command = 'slidePosition';
                const stateId = `${this.adapter.namespace}.${deviceId}.${_command}`;
                const output = await this.adapter.getStateAsync(stateId); 
                value = output.val;
            default:
             // 1. For security reason turnOff all relais first
                //await this.api.sendCommand(deviceId, { command: 'turnOff', parameter: 'all', commandType: 'command' });
                await this.handleBlindsPause(deviceId, 'stop(1,2)');
                commandData = null;
                
             // 2. Map command to API format using CommandMapper
                commandData = this.commandMapper.mapCommand(command, value);
                if (!commandData) {
                    this.log.warn(`Unknown command: ${command} for device ${deviceId}`);
                }
        ...
        if(commandData) {
            await this.api.sendCommand(deviceId, commandData);
            this.log.info(`Command ${command} sent to device ${deviceId}`);
            // reset Button in ioB-state
            const stateId = `${this.adapter.namespace}.${deviceId}.${command}`;
            await this.adapter.setStateAsync(stateId, false, true); // val: false, ack: true
        }
        
        // Set 120-Sec. Auto-Off Timer for open/close  (Fallback add. internal calibration)
        if (command === 'open' || command === 'close' || command === 'setBlindPosition') {
            const timer = setTimeout(async () => {
                try {
                    this.log.debug(`Safety Auto-Off triggered for ${deviceId} after 120s`);
                    await this.handleBlindsPause(deviceId, 'stop(1,2)');
                    commandData = null;
                } catch (err) {
                    this.log.error(`Safety Auto-Off failed: ${err.message}`);
                }
            }, 120000); // 120 seconds
            this.relayTimers.set(deviceId, timer);
         }
    }
     /**
     * Handle blinds pause command
     * @param {string} deviceId - Device identifier
     * @param {string} command - Command name to execute
     */
    async handleBlindsPause(deviceId, command) {
        let commandData = null;
        try {
            switch(command) {
                case('stopAll'):
                    commandData = this.commandMapper.mapCommand('stop', 'all'); //('setOff', 'all');
                    if(commandData) {
                        await this.api.sendCommand(deviceId, commandData);
                        this.log.info(`Command ${command} sent to device ${deviceId}`);
                    }else {
                ...
                default:
                    // if all not works
                    commandData = this.commandMapper.mapCommand('setOff', '1');
                    if(commandData) {
                        await this.api.sendCommand(deviceId, commandData);
                        this.log.info(`Command ${command} sent to device ${deviceId}`);
                    }
                     commandData = this.commandMapper.mapCommand('setOff', '2');
                    if(commandData) {
                        await this.api.sendCommand(deviceId, commandData);
                        this.log.info(`Command ${command} sent to device ${deviceId}`);
                    }
            ...
            this.log.error(`Setting Pause failed: ${err.message}`);
        }
        // 2. reset IoB SP
        // reset Button in ioB-State
        const _command = 'pause';
        const stateId = `${this.adapter.namespace}.${deviceId}.${_command}`;
        await this.adapter.setStateAsync(stateId, false, true); // val: false, ack: true
