...
class DeviceManager {
    ...
    async handleBlindsCommand(deviceId, command, value) {
        ...
        // Alten Timer löschen, falls das Rollo gerade fährt und ein neuer Befehl kommt
        ...
        switch (command) {
            case('open'):
            commandData = { command: 'turnOn', parameter: '1', commandType: 'command' };
            break;
        case('close'):
            commandData = { command: 'turnOn', parameter: '2', commandType: 'command' };
            break;
        case('pause'):
            // PAUSE: Sende explizit turnOff an BEIDE Kanäle nacheinander
            await this.api.sendCommand(deviceId, { command: 'turnOff', parameter: '1', commandType: 'command' });
            await this.api.sendCommand(deviceId, { command: 'turnOff', parameter: '2', commandType: 'command' });
            this.log.info(`Pause command (turnOff 1 & 2) sent to device ${deviceId}`);
            return; // Beendet die Funktion hier, da Befehle bereits gesendet wurden
        case('setBlindPosition'):
            commandData = { command: 'setPosition', parameter: String(value), commandType: 'command' };
        }

        // 120-Sekunden Auto-Off Timer für open/close setzen (Fallback zur internen Kalibrierung)
        if (command === 'open' || command === 'close' || command === 'setBlindPosition') {
            const timer = setTimeout(async () => {
                try {
                    this.log.debug(`Safety Auto-Off triggered for ${deviceId} after 60s`);
                    await this.api.sendCommand(deviceId, { command: 'turnOff', parameter: '1', commandType: 'command' });
                    await this.api.sendCommand(deviceId, { command: 'turnOff', parameter: '2', commandType: 'command' });
                } catch (err) {
